...
public class TestGestor {
    ...
    private static GestorPista gestorPista;
    private static GestorEstacio gestorEstacio;
    private static GestorPistaAlpina gestorPistaAlpina;
    private static GestorPistaNordica gestorPistaNordica;
    private static GestorItinerariRaquetes gestorItinerariRaquetes;
    ...
    public static void setUpClass() {
        emf = Persistence.createEntityManagerFactory("eac2PU");
        em = emf.createEntityManager();

        gestorPista = new GestorPista(em);
        gestorEstacio = new GestorEstacio(em);
        gestorPistaAlpina = new GestorPistaAlpina(em);
        gestorPistaNordica = new GestorPistaNordica(em);
        gestorItinerariRaquetes = new GestorItinerariRaquetes(em);
    }

    @BeforeEach
    public void setUpTest() throws GestorExcepcio {
        em.getTransaction().begin();
        em.createQuery("DELETE FROM Pista").executeUpdate();
        em.createQuery("DELETE FROM Estacio").executeUpdate();
        ...
        for (Estacio estacio : estacions) {
            gestorEstacio.afegeixEstacio(estacio);
        }
    }
    ...
    public void testEsborraEstacio(String idEstacio) throws GestorExcepcio {
        ...
        assertNull(em.find(Estacio.class, idEstacio));
        assertThrows(GestorExcepcio.class, () -> gestorEstacio.esborraEstacio("E-XX"));
    }
    ...
    public void testEsborraPista() throws GestorExcepcio {
        List<String> idsPistes = pistes.stream().map(Pista::getId).toList();

        for (String idPista : idsPistes) {
            Pista pista = em.find(Pista.class, idPista);
            Estacio estacio = pista.getEstacio();
            String idEstacio = estacio.getId();

            List<Pista> pistesRestants = estacio.getPistes().stream()
                    .filter(p -> !p.getId().equals(idPista))
                    .toList();
            double percentatgeEsperat;
            if (pistesRestants.isEmpty()) {
                percentatgeEsperat = 0.0;
            } else {
                long obertes = pistesRestants.stream().filter(Pista::isOberta).count();
                percentatgeEsperat = (obertes * 100.0) / pistesRestants.size();
            }

            gestorPista.esborraPista(idPista);
            em.clear();

            assertNull(em.find(Pista.class, idPista));

            Estacio estacioActualitzada = em.find(Estacio.class, idEstacio);
            assertEquals(percentatgeEsperat, estacioActualitzada.getPercentatgePistesObertes(),
                    "El percentatge d'obertura d'E-" + idEstacio + " no s'ha actualitzat correctament despres d'esborrar " + idPista);
        }

        for (String idPista : idsPistes) {
            assertThrows(GestorExcepcio.class, () -> gestorPista.esborraPista(idPista));
        }
    }

    @Test
    public void testActualitzaGruixNeuPista() throws GestorExcepcio {
        int abans = em.find(Pista.class, "PA-02").getGruixNeu();
        gestorPista.actualitzaGruixNeuPista("PA-02", 7);
        em.clear();
        int despres = em.find(Pista.class, "PA-02").getGruixNeu();

        assertEquals(abans + 7, despres);
        assertThrows(GestorExcepcio.class, () -> gestorPista.actualitzaGruixNeuPista("PA-XX", 4));
    ...
    public void testActualitzaObertaPista(String idPista, boolean oberta, double percentatgeEsperat) throws GestorExcepcio {
        ...
        Pista pistaActualitzada = em.find(Pista.class, idPista);
        Estacio estacio = em.find(Estacio.class, pistaActualitzada.getEstacio().getId());
